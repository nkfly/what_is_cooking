...
def train_json2matrix():
	...
	for k1, v1 in trainData.items():
		...
		data.append(depCount)
		row.append(r)
		col.append(7+69+5354+1+1+1+1+1-1)

		answer.append(visitNumber2tripType[k1])
		count += 1
	# Create the COO-matrix
	coo = coo_matrix((data,(row,col)), shape=(len(trainData), 7+69+5354+1+1+1+1+1))
...
def test_json2matrix():
	...
	for k1, v1 in testData.items():
		...
		count += 1
	# Create the COO-matrix
	coo = coo_matrix((data,(row,col)), shape=(len(testData), 7+69+5354+1+1+1+1+1))
	# Let Scipy convert COO to CSR format and return
